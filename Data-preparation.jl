...
# ╔═╡ 163d15ed-5f3c-4b02-af6a-40a0b9e5b156
using Random

# ╔═╡ 2c920a2e-38a5-45c7-b62e-d4d113d71f39
using JuliaHubData, DataSets, CSV, DataFrames

# ╔═╡ d931fdad-1645-4956-a982-88801161a96f
using MLJ

# ╔═╡ 0ca525b1-5d50-43de-9cf8-d29424477fbe
using Flux, Plots

# ╔═╡ 3e0baa1e-2a29-11ee-1be0-cb7fd28bbd6a
md"""
Working with **vectors**: generation, train/test split
"""

# ╔═╡ 152de0de-ba36-4f23-b35d-b3d158d5a015
begin
	g(x, y) = exp(-(x^2 + y^2)/2) # Underlying function
	
	xr = [-1:0.05:1;] # range for x
	yr = [-1:0.05:1;] # range for y
...
# ╔═╡ 46d3b0ba-d6a8-4749-8626-5ee8a87065fe
begin
	a = [1, 5, 0, -9]
	ind = randperm(4)
	println(ind)
...
# ╔═╡ 866bec33-fb90-497d-b12c-c534d7ad1431
begin
	# reorganization of data
	input_data = [[x, y] for (x,y) in zip(Float32.(X), Float32.(Y))] # the input should be a 2d vector
	output_data = Float32.(Z)
	println("first input: ", input_data[1])
	println("first output: ", output_data[1])
	...
	output_data_test = output_data[[ntrain+1:1:nobs;]]

	data_train = [(input_data_train, output_data_train)]
	data_test = [(input_data_test, output_data_test)]

	println("input_data_train is a vector of  " , length(input_data_train), " bidimensional vectors")
	println("input_data_test is a vector of  " , length(input_data_test), " bidimensional vectors")

end

# ╔═╡ 15c1e998-d1af-402b-a5e4-1bdb1a11d0ee
md"""
Working with **Data Frames** from csv files: read, train/test split
"""

# ╔═╡ 7c48243b-52f5-47e2-90a9-23531f312908
diabetes = open(Vector{UInt8}, dataset("msrosito/diabetes")) do buf
    CSV.read(buf, DataFrame)
end

# ╔═╡ 45d3e949-06bf-44e9-b74c-b08d8925fbcd
diabetes_train, diabetes_test = MLJ.partition(diabetes, 0.75, shuffle = true)

# ╔═╡ a3a2b91b-d535-40dd-bb65-32ec2d457017
begin
	println("Diabetes data set size: ", size(diabetes))
	println("Diabetes train data set size: ", size(diabetes_train))
	println("Diabetes test data set size: ", size(diabetes_test))
end

# ╔═╡ 2d9864c8-b01c-4530-93c4-61ced6ed535d
md"""
**Example**
"""

# ╔═╡ 671d504c-7baf-42a2-bd29-8d7d3c61f542
begin
	nfeatures = size(diabetes_train)[2] - 1
	diabetes_train[:, 1:nfeatures] #data set without the last column
end

# ╔═╡ f8980714-dc5e-4f81-a1da-985d8f53de86
Vector(diabetes_train[1, 1:nfeatures]) ## first row as vector (without the last column)

# ╔═╡ 3184ac41-e333-4830-b2cb-80503b9e317e
begin
	diabetes_ntrain = size(diabetes_train)[1]
	x_train = [ Vector(diabetes_train[i, 1:nfeatures]) for i in 1:diabetes_ntrain] 
end

# ╔═╡ f3abd1bb-8360-4651-9118-8a1367cc8313
diabetes_train[:, nfeatures + 1]  # last column 

# ╔═╡ ed13c8a5-ea5f-4bf1-b945-5b1b2cf047cd
md"""
The same steps should be repeated for the test data set
"""

# ╔═╡ 67866f57-3755-4212-b5b5-9fa0443210ae
md"""
**Example**
"""

# ╔═╡ d9ce3a2d-baad-4d6a-b1ad-740ca1722fc3
begin
	# building the model
	insize, outsize = 2, 1
...
# ╔═╡ ca36046c-6bd3-433f-aca6-64af7caceeb2
begin 
	# defining loss
...
# ╔═╡ 9fec28b3-0637-4d97-b780-c65cab42e8ca
md"""
Check the addition of **losses_test**
"""

# ╔═╡ d1ba2181-2f86-4210-be5a-67b346f14aa3
begin
	# training (pay attention to the syntax)
	
	losses_train = [] # history of losses (training)
	losses_test = [] # history of losses (test)
...
# ╔═╡ e45d41d4-b164-4901-b757-7f738ad24fba
begin
	plot(losses_train, label = "training set")
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "2443bb4211923f2d50e46d0383bc756f22e01d2a"
