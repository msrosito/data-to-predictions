...
# ╔═╡ 0e8620f8-2d58-11ee-0597-f7f2f5d496d9
using Plots, Flux

# ╔═╡ 6fc4284f-b51b-4ad1-b6c7-73142cb78c94
...
# ╔═╡ 4fb88770-b0f5-43f1-83fa-4175b57ad94f
n, m = 100, 100

# ╔═╡ f81fa7a0-10c6-4041-8849-521a1beaf98e
md"""
Generation of the training set
"""

# ╔═╡ 8ef35127-f47e-48cc-8763-755ae8021bb2
ntrain = 400

# ╔═╡ 86d72269-b25f-47a8-8c32-490616157d13
begin
	x_train = [] # images or matrices
	y_train = [] # "normalized angles"
	for _ in 1:ntrain
		road, angle = gen_road_angle(n, m)
		push!(x_train, reshape(Float32.(road), (n, m, 1, 1)))
		push!(y_train, angle)
	end
	data_train = [(x_train, y_train)]
end

# ╔═╡ 8c8d06a5-b67c-481a-a684-86ff89b71562
md"""
Generation of the test set
"""

# ╔═╡ 1925f941-f02e-4a18-9368-8bbe029c5bad
ntest = 100

# ╔═╡ 4cd13c14-806f-44eb-9513-8e9a16dcb8f7
begin
	x_test = [] # images or matrices
	y_test = [] # "normalized angles"
	for _ in 1:ntest
		road, angle = gen_road_angle(n, m)
		push!(x_test, reshape(Float32.(road), (n, m, 1, 1)))
		push!(y_test, angle)
	end
	data_test = [(x_test, y_test)]
end

# ╔═╡ aa8e9b1b-4038-4a2c-9fb2-735ab98510bc
md"""
Definition of the CNN
"""

# ╔═╡ 673d82c7-9710-4dbf-9bac-7782248bf3c6
md"""
* Conv((5, 5), 1=>6, relu): Kernel size = (5, 5), Input = 1 matrix, Output = 6 matrices (because there are 6 kernels), activation function = relu

* MaxPool((2, 2)). Output: 6 smaller images

* Conv((5, 5), _=>16, relu). Kernel size = (5,5), Input = 6 matrices (automatically computed), Output = 6 * 16 matrices (because there are 16 kernels), activation function = relu

* MaxPool((2, 2)). Output: 6 * 16 = 96  smaller images

* Flatten: one vector

* Dense(_ => 120, relu): Input: size of the vector, Output = 120, activation = relu

* Dense(_ => 120, relu): Input: size of the previous ouput (120), Output = 84, activation = relu

* Dense(_ => 1, relu): Input: size of the previous ouput (84), Output = 1, activation = linear


"""

# ╔═╡ 409da9e1-e0b4-48e2-a3a5-0303345ff8f8
lenet = Flux.@autosize (n, m, 1, 1) Chain(
    Conv((5, 5), 1=>6, relu, init = Flux.glorot_normal),
    MaxPool((2, 2)),
    Conv((5, 5), _=>16, relu, init = Flux.glorot_normal),
    MaxPool((2, 2)),
    Flux.flatten,
    Dense(_ => 120, relu, init = Flux.glorot_normal),
    Dense(_ => 84, relu, init = Flux.glorot_normal), 
    Dense(_ => 1, init = Flux.glorot_normal),
)

# ╔═╡ 409da674-66ca-4a29-a676-564008e6a575
lenet(x_train[1])

# ╔═╡ 579da25c-7121-4a7d-a8ff-c9ee20b107e7
md"""
Training
"""

# ╔═╡ 754e4196-a72c-43a1-a311-7ec7281ddc0c
begin
	# defining loss
...
#	optimizer = Descent(0.2) # 0.2 is the learning rate
	optimizer = Flux.setup(Adam(3e-4), lenet)
	
	n_epoch = 100
end

# ╔═╡ 23df9124-b3d4-4388-ae4d-835861ebceb7
begin

	losses_train = []
	
	for _ in 1:n_epoch
		Flux.train!(loss, lenet, data_train, optimizer)
		l = loss(lenet, x_train, y_train)
		push!(losses_train, l)
	end
end

# ╔═╡ 515bb410-c4d3-42a3-92d8-3e97cbc70e75
plot(losses_train)

# ╔═╡ 8b46618a-3188-4007-90fb-1b54ef04bd20
begin
	predicted_values = first.(lenet.(x_train))
	println(predicted_values)
	println(y_train)
	println("Training loss: ", loss(lenet, x_train, y_train))
	println("Training loss: ", loss(lenet, x_test, y_test))
end

# ╔═╡ 1573f978-e04f-4f0a-b46c-fb618b0368de
md"""
Recovering an image
"""

# ╔═╡ 1ad22128-838b-48e6-b035-d540d571fae1
...
# ╔═╡ d87d2f38-5647-4ea4-a6b6-82b1d926705f
md"""
Generation of a new sample
"""

# ╔═╡ e508635c-acc1-4b95-ac0f-465bd8890721
new_road, new_angle = gen_road_angle(n, m)

# ╔═╡ 55beb105-b0ae-42dc-96e9-065906c36c3c
heatmap(new_road, legend = false, title = "Norm. angle: $new_angle", c = :grayC)

# ╔═╡ f0ba4ab2-9b31-4a6b-a114-25f8aef33db6
md"""
Prediction using this new sample
"""

# ╔═╡ bc18328e-b5b0-49c3-bf5f-46a845eb074c
begin
	new_road2 = reshape(Float32.(new_road), (n, m, 1, 1))
	pred = first(lenet(new_road2))
	println("True value: ", new_angle)
	println("Predicted value: ", pred)
end

# ╔═╡ 86e56c4b-2686-4e82-9221-1acd10fc44ca
generate_image(n, m, pred)
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "aa371fe53107b1a2cab6e8a40940a74f1bf66ebb"
