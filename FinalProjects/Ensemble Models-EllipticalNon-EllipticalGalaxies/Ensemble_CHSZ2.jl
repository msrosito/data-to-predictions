...
# ╔═╡ 9bc99b0a-2e72-11ee-21c0-6d8bd237f0c3
using CSV, Plots, DataFrames, DataSets, Flux, JuliaHubData, MLJ, Random

# ╔═╡ f4d9dfae-e413-4f3f-9fea-965927d3e520
galaxies = open(Vector{UInt8}, dataset("msrosito/FenixGalaxies")) do buf
		CSV.read(buf, DataFrame, header = false)
	end

# ╔═╡ a95a83f5-624d-486d-85d3-a78258eab152
function compute_accuracy(pred_prob, true_labels, threshold) # we should provide both, the predicted probabilities of diabetes and the true labels
	nobs = length(pred_prob)
	pred_labels = []
	for i in 1:nobs
		if pred_prob[i] < threshold
			push!(pred_labels, 0) # non elliptical
		else
			push!(pred_labels, 1) # elliptical
		end
	end
	return pred_labels, 1 - sum(abs.(pred_labels - true_labels))/nobs # notice that the second term is counting the misclassifications
end

# ╔═╡ e9d39888-42ff-4df0-b443-65d001b571c7
data_train, data_test = MLJ.partition(rng = 123, galaxies, 0.7, shuffle = true)

# ╔═╡ 6c863bf7-298a-437c-8f28-5260d3afca99
size(galaxies)

# ╔═╡ 011c8572-a9f2-4b2b-a47c-936ae43334f5
nfeatures = size(galaxies)[2] - 1

# ╔═╡ 70f8c795-5def-4c50-9633-5896987aee96
begin
	galaxies2 = open(Vector{UInt8}, dataset("msrosito/FenixGalaxies")) do buf
	CSV.read(buf, DataFrame)
	end
	size(galaxies2) ### check that you lose a row
end

# ╔═╡ 78608042-5966-4085-9124-70282664cb37
begin
	## training set
	ntrain = size(data_train)[1] # number of observations in the training data set
	x_train = [ Float32.(Vector(data_train[i, 1:nfeatures])) for i in 1:ntrain ] # we are choosing the first nfeatures columns to discard the column of labels
	y_train = Float32.(data_train[:, nfeatures + 1]) # last column as a vector

	data_train_mlp = [(x_train, y_train)]
end

# ╔═╡ 34657026-3adb-4dd2-b936-cb5f7362158a
begin
	## test set
	ntest = size(data_test)[1] # number of observations in the test data set
	x_test = [ Float32.(Vector(data_test[i, 1:nfeatures])) for i in 1:ntest] # we are choosing the first nfeatures columns to discard the column of labels
	y_test = Float32.(data_test[:, nfeatures + 1]) # last column as a vector

	data_test_mlp = [(x_test, y_test)]
end

# ╔═╡ a32ffcb2-c19b-4cb1-8b40-248918dc2ea1
begin
	threshold = 0.5
	ins = nfeatures
	outs = 1 # one dependent variable
end

# ╔═╡ ec08b46b-77be-4ecc-8417-247f7bc396bc
loss(model, x, y) = Flux.logitbinarycrossentropy(first.(model.(x)), y)

# ╔═╡ 9e2a5e6a-e55a-4561-8480-497c2fc46d68
function training!(model, optimizer, nepoch, x_train, y_train)
...
# ╔═╡ 2485054f-3e4c-4e75-a31b-935d538e2837
md"""
Model 1
"""

# ╔═╡ 973f65ed-fea4-4f3c-ad70-9111c4c05d9f
begin

	r1 = MersenneTwister(120)
	model1 = Chain(Dense(ins, 15, relu; init = Flux.glorot_normal(r1)),

		Dense(15, 15, relu; init = Flux.glorot_normal(r1)),
		
		Dense(15, outs, σ; init = Flux.glorot_normal(r1))) 

	println(Flux.params(model1))

	optimizer1 = Flux.setup(Adam(0.0003), model1)

	nepoch1 = 50

	losses1_train, losses1_test = training!(model1, optimizer1, nepoch1, x_train, y_train)

	println("\n", Flux.params(model1))

end

# ╔═╡ 79d526c5-fe31-4ed5-8e3d-2692e3faae5e
begin
	plot(losses1_train, label = "training")
	plot!(losses1_test, label = "test")
end

# ╔═╡ 9702092c-2f67-40c7-8b6f-79d662982bc6
begin
	predicted_probabitities_train1 = first.(model1.(x_train))
	predicted_probabitities_test1 = first.(model1.(x_test))
end

# ╔═╡ 8f355ec4-737d-4340-abeb-e52354b016d2
begin
	predicted_labels_train1, acc_train1 = compute_accuracy(predicted_probabitities_train1, y_train, threshold)
	println("Training accuracy for model 1: ", acc_train1)
	predicted_labels_test1, acc_test1 = compute_accuracy(predicted_probabitities_test1, y_test, threshold)
	println("Test accuracy for model 1: ", acc_test1)
end

# ╔═╡ d7e88393-8935-4725-82c6-0c5e56748a96
md"""Model 1a"""

# ╔═╡ 901ece6d-d260-4744-8097-cfd8d7c64cd1
begin

	r1a = MersenneTwister(120)
	model1a = Chain(Dense(ins, 15, relu; init = Flux.glorot_normal(r1a)),

		Dense(15, 15, relu; init = Flux.glorot_normal(r1a)),
		
		Dense(15, outs, σ; init = Flux.glorot_normal(r1a))) 

	println(Flux.params(model1a))

	optimizer1a = Flux.setup(Adam(0.0003), model1a)

	nepoch1a = 30

	losses1a_train, losses1a_test = training!(model1a, optimizer1a, nepoch1a, x_train[1:ntrain÷2], y_train[1:ntrain÷2])

	println("\n", Flux.params(model1a))

end

# ╔═╡ 4fb5c287-f050-4761-a7e3-2013fba42de1
begin
	plot(losses1a_train, label = "training")
	plot!(losses1a_test, label = "test")
end

# ╔═╡ 409896ec-ba08-441e-ad63-bb1c90c00f77
begin
	predicted_probabitities_train1a = first.(model1a.(x_train[1:ntrain÷2]))
	predicted_probabitities_test1a = first.(model1a.(x_test))
end

# ╔═╡ 28d621f1-6fb1-4530-91ff-0ff85c7bc2ac
begin
	predicted_labels_train1a, acc_train1a = compute_accuracy(predicted_probabitities_train1a, y_train[1:ntrain÷2], threshold)
	println("Training accuracy for model 1a: ", acc_train1a)
	predicted_labels_test1a, acc_test1a = compute_accuracy(predicted_probabitities_test1a, y_test, threshold)
	println("Test accuracy for model 1a: ", acc_test1a)
end

# ╔═╡ 7aa164f5-31a3-4c78-9737-3fe986fa2b6f
md"""Model 1b"""

# ╔═╡ ea99f205-d17a-4b47-a59c-7f923f6e643d
begin

	r1b = MersenneTwister(120)
	model1b = Chain(Dense(ins, 15, relu; init = Flux.glorot_normal(r1b)),

		Dense(15, 15, relu; init = Flux.glorot_normal(r1b)),
		
		Dense(15, outs, σ; init = Flux.glorot_normal(r1b))) 

	println(Flux.params(model1b))

	optimizer1b = Flux.setup(Adam(0.0003), model1b)

	nepoch1b = 75

	losses1b_train, losses1b_test = training!(model1b, optimizer1b, nepoch1b, x_train[ntrain÷2+1:ntrain], y_train[ntrain÷2+1:ntrain])

	println("\n", Flux.params(model1b))

end

# ╔═╡ db0290b2-9ce1-409d-b6d8-adee7e11e588
begin
	plot(losses1b_train, label = "training")
	plot!(losses1b_test, label = "test")
end

# ╔═╡ 85cd9eb7-2923-4db6-afac-27cdd87470a3
begin
	predicted_probabitities_train1b = first.(model1b.(x_train[ntrain÷2+1:ntrain]))
	predicted_probabitities_test1b = first.(model1b.(x_test))
end

# ╔═╡ 1dfd23cf-5482-4be6-b97c-35caa3f25a58
begin
	predicted_labels_train1b, acc_train1b = compute_accuracy(predicted_probabitities_train1b, y_train[ntrain÷2+1:ntrain], threshold)
	println("Training accuracy for model 1b: ", acc_train1b)
	predicted_labels_test1b, acc_test1b = compute_accuracy(predicted_probabitities_test1b, y_test, threshold)
	println("Test accuracy for model 1b: ", acc_test1b)
end

# ╔═╡ f043b1c9-b449-4199-8250-25dd0ed9be7b
md"""
Ensemble Model 1a + Model 1b
"""

# ╔═╡ 4995f1e6-081d-4d1a-a756-4eeb9373c9bf
predicted_probabitities_test_ens1ab_avg = (predicted_probabitities_test1a + predicted_probabitities_test1b) /2

# ╔═╡ e685be19-d7d8-4774-b483-80a6b55e537c
predicted_labels_test_ens1ab_avg, acc_test_ens1ab_avg = compute_accuracy(predicted_probabitities_test_ens1ab_avg, y_test, threshold)

# ╔═╡ 7176520b-3a5b-431f-bf91-465ffb7cc282
println("Test accuracy for the ensemble model 1ab: ", acc_test_ens1ab_avg)
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "b723aafbaac3b7518b4d7d1e066e9f1240feb047"
