...
# ╔═╡ 8cbc90d4-2bca-11ee-1a50-75496138e797
using CSV, DataSets, JuliaHubData, DataFrames, GLM, MLJ

# ╔═╡ a73ba0c4-4a3e-4cd8-ae3d-f4199c97d56a
galaxies = open(Vector{UInt8}, dataset("msrosito/FenixGalaxies")) do buf
CSV.read(buf, DataFrame, header = false)
end

# ╔═╡ c7da3c44-8ab4-44ff-8bc4-2dfb8b7d27c8
rename!(galaxies, :Column3073 => :label) # not necessary, just to establish that the last column is the label (non-elliptical/elliptical). Notice: "!"

# ╔═╡ 03c32e61-6f1b-44d0-b8e5-c1290bb451f7
gal_train, gal_test = MLJ.partition(galaxies, rng = 123, 0.8, shuffle = true ) # rng is to make the split deterministic

# ╔═╡ b07c6403-5e2c-4f89-84e5-10d1283af9fd
md"""
size(galaxies) returns the size of the Data Frame. The first value is the number of rows, the second the number of columns. Therefore, the number of features is the number of columns - 1 (because we have one response).
"""

# ╔═╡ 446e46e6-f156-4054-9412-1dee41b254f9
begin
	println("Size of the data set", size(galaxies))
	nfeatures = size(galaxies)[2] - 1 
	ntrain = size(gal_train)[1]
	ntest = size(gal_test)[1]
end

# ╔═╡ 24aa759d-b838-4a7c-b860-0c2c0ceb4f58
md"""
...
# ╔═╡ 70f3f91e-adc7-46d9-8a1a-2b88ae3b589e
some_columns = [:Column1, :Column2, :Column10]

# ╔═╡ 9137986f-0a61-4693-a4a7-5d7e498924c6
terms1 = vcat([1], some_columns) # concatenation: adds the term 1

# ╔═╡ 67d71784-6e6e-4978-8c76-6f4e8d99e426
md"""
Compact form
"""

# ╔═╡ 20d16b7b-0aef-4039-9486-0cd009385f06
form1 = Term(:label) ~ sum(term.(terms1))

# ╔═╡ 871bbbef-e90a-4d81-9f66-ccf73429e355
md"""
Equivalence
"""

# ╔═╡ a074f73b-378f-41b3-99d3-5f058c3eb420
begin
	equiv_form1 = @formula(label ~ 1 + Column1 + Column2 + Column10)
	equiv_form1 == form1
end

# ╔═╡ d00bbf70-e8a4-486d-83ee-ff727294b857
md"""
So, if we want all the columns:
"""

# ╔═╡ 246085fd-76af-4c5b-98d4-021a2e2d763c
propertynames(galaxies) ## names of the columns

# ╔═╡ 4d453856-6eab-4738-8f85-b4fc032f2d7d
names = propertynames(galaxies)[1:nfeatures] # remove the last element

# ╔═╡ 198c2651-feb8-45fe-9880-e000966dd7f2
terms2 = vcat([1], names) # concatenation

# ╔═╡ b9d8686a-d57f-42f5-9b54-6c3dc549be40
form2 = Term(:label) ~ sum(term.(terms2))
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "5f95e2d65b275ade2de9f1a822009055447322b2"
